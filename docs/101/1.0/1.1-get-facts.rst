...
~~~~~~~
  ...
  A play is a list of tasks. Tasks and modules have a 1:1 correlation.  Ansible modules are reusable, standalone scripts that can be used by the Ansible API, or by the ansible or ansible-playbook programs. They return information to ansible by printing a JSON string to stdout before exiting.
    ...
    -  The ``server: "{{ ansible_host }}"`` parameter tells the module to connect to the F5 BIG-IP IP address, which is stored as a variable in the lab inventory.
    -  The ``user: "{{ ansible_user }}"`` parameter tells the module the username to login to the F5 BIG-IP device with, which is stored as a variable in the lab inventory.
    -  The ``password: "{{ ansible_password }}"`` parameter tells the module the password to login to the F5 BIG-IP device with, which is stored as a variable in the lab inventory.
    -  The ``server_port: {{ server_port }}`` parameter tells the module the port to connect to the F5 BIG-IP device with, which is stored as a variable in the lab inventory.
...
~~~~~~~
  ...
  .. code:: yaml
    ...
    [rhel-user@ede7a345-c0f1-47f9-a73b-74fded8ec113 ~]$ ansible-navigator run bigip-info.yml --mode stdout
